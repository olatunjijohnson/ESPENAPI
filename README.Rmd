...
**ESPENAPI** gives R users access to Neglected Tropical Disease (NTD) data
from two sources:
...
See the [WHO GHO section](#who-gho-api-no-key-required) below for full details.
...
## WHO GHO API — no key required {#who-gho-api-no-key-required}

The WHO [Global Health Observatory](https://www.who.int/data/gho/info/gho-odata-api)
OData API is publicly accessible with no authentication. The package
provides `gho_ntd_data()` as a no-key alternative for onchocerciasis and
trachoma data.
...
```{r gho-indicators-table, echo = FALSE, eval = TRUE, results = "asis"}
ind <- ESPENAPI::gho_ntd_indicators()
knitr::kable(
  ind[ind$gho_available, c("espen_code", "gho_code", "gho_name")],
  col.names = c("ESPEN disease", "GHO code", "Description"),
  row.names = FALSE
)
```
...
Your key is invalid, expired, or new keys are not currently being issued.
Try the WHO GHO alternative (`gho_ntd_data()`) or contact
<ntd.espen@who.int> directly.

**`API did not return JSON`**
The ESPEN API endpoint has changed or is temporarily unavailable.
The current endpoint is `https://espenjapapi.afro.who.int/api/data`.
